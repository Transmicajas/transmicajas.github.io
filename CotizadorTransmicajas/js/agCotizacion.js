  ...
  filas.forEach((fila) => {
    ...
    if (celdas.length >= 9) {
      const producto = {
        ...
        marca: celdas[5].textContent.trim(),
        ubicacion: celdas[6].textContent.trim(),
        referencia: celdas[2].textContent.trim(),
        precioCompra: parseMoney(celdas[7].textContent.trim()),
        precioVenta: parseMoney(celdas[8].textContent.trim()),
      };
  ...
  productosGuardados.forEach((producto, index) => {
    ...
    fila.innerHTML = `
      ...
      <td class="text-center acciones-columna">
        <button class="editar-producto-cotizacion">Editar</button>
        <button class="eliminar-producto-cotizacion">Eliminar</button>
      </td>
    `;
    ...
    if (isMobileDevice()) {
      ...
      fila
        .querySelector(".editar-producto-cotizacion")
        .addEventListener("click", () => {
          console.log("Click en botón editar - index:", index);
          editarProducto(index);
        });

      fila
        .querySelector(".eliminar-producto-cotizacion")
